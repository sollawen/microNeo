...
// Position — 说明-AIBP §5.3，被 Cursor / Selection.Start / Selection.End 复用。1-based（行从 1 起、列从 1 起）
type Position struct {
	Line int `json:"line"`
	Col  int `json:"col"`
}

// ContextPayload — 说明-AIBP §5.3 `context` 报文 payload
type ContextPayload struct {
	Path         string     `json:"path"`
	Cursor       Position   `json:"cursor"`
	Selection    *Selection `json:"selection,omitempty"`    // 无选区则省略（不是 null）
	Message      string     `json:"message,omitempty"`      // 有无决定递送路径（说明-AIBP §六）
...
// MarshalLine 序列化为单行 JSON + \n（说明-AIBP §4.3 分帧）。调用方写入 socket 后关闭连接。
