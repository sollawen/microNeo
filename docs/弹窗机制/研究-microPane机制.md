...
## 一、micro 的事件循环：DoEvent 是什么（必读基础）

> 本节是后续所有讨论的**事实基础**。先看懂本节再读 §二的核心结论。

micro 是一个**事件驱动**的程序。它的核心是一个事件循环 `DoEvent()`，位于 `cmd/micro/micro.go`。每帧做三件事：

1. **画**（Display 阶段）：把当前所有"该画的东西"按顺序画到屏幕
2. **等事件**（Event 阶段）：从屏幕读一个事件（或从其它 channel 读信号）
3. **处理事件**：按规则分发给某处处理
...
func DoEvent() {
    ...
    action.Tabs.Display()                              // ① Tab 栏
    for _, ep := range action.MainTab().Panes {
        ep.Display()                                   // ② 各 pane（BufPane）
    }
    action.MainTab().Display()                         // ③ main tab 自身
    action.InfoBar.Display()                           // ④ 底部 info bar（永远在）
...
**关键观察**（不熟 micro 的读者必须理解的三件事）：

1. **Display 阶段是"无脑按顺序画"**——顺序：Tabs → 各 pane → InfoBar → notePane（如果开着）。**后画的盖在先画的上**（z-order 由调用顺序决定）
2. **事件 if-else 树是"短路"**——`notePane.IsOpen()` 命中 → 事件给 notePane（其它一律不收）。**这就是 micro 实现"浮窗 modal"的方式**
3. **notePane 是 micro 实现"浮窗"的模板**——Display 阶段追加一段 + 事件 if-else 树里插一条 if 分支。selectPane 现状（d13 scaffolding）没用这个模式，而是走 bufpane forwarding（见 [`弹窗框架设计.md` §1.4](./弹窗框架设计.md#14-bufpane-的-forwarding方案-a-的现状)）
...
## 五、micro 现有 pane 的实现方式（4 个 + 1 个反例）
...
`internal/action/selectpane.go`：只实现 5 个方法（Open/Close/IsOpen/HandleEvent/Display），不嵌入 BufPane，不实现 Pane 接口（5/16）。靠**宿主 pane 的 hook 转发**收事件和绘制（详见 §七）。

---

## 六、事件路由：DoEvent 的 if-else 树（关键）
...
| 满足 Pane 接口 = 使用 Pane 接口 | ❌ 详见 §八。InfoPane/NotePane 满足但不使用 |
| StatusLine 是寄生的 pane | ❌ StatusLine 根本不是 pane（详见 §8.5） |
| 进 DoEvent 的浮层都靠 Pane 接口路由 | ❌ notePane/InfoBar 走具体类型方法，不经 Pane 接口（详见 §八） |
