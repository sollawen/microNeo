...
type AgentEnsurer interface {
    // ... 现有 5 个方法（AgentName / HasAgent / AIBPVersion / InstallAIBP / UpdateAIBP）不变 ...
...
func (OpencodeEnsurer) UpdateToLatest(report Reporter) error {
    ...
    _, _, isSource := (OpencodeEnsurer{}).AIBPVersion()

    // npm 已装 → 查 registry，新了才装；已是最新则跳过
    if !isSource {
        if _, _, installed, _ := opencodeNpmAIBPVersion(); installed != "" {
            latest, err := npmRegistryLatest(aibpOpencodeSpec)
            if err != nil {
...
对现有代码仅两处微调，**零新增 helper**：

1. **`opencodeRemoveTuiEntries()` 谓词加一行**，让它连源码路径条目（含 `aibp-agents`）一起删。于是 `installOrUpdate()`（经 `uninstallAIBP()` → `opencodeRemoveTuiEntries()`）自动完成 source→npm：先删源码条目、再重装 npm，无需另造 source 专用卸载函数。npm→npm 路径没有源码条目，新分支永不命中，行为不变：
...
2. **`opencodeNpmAIBPVersion()` 顺带返回 `.version`**——它本来就读这个 `package.json`（现只取 `.aibp.protocol`）。签名加一个 `version string` 返回值；现有调用方 `AIBPVersion()` 丢弃不用，接口签名不变。「读已装版本」因此零新增、零重复读文件。
...
func (ClaudeEnsurer) UpdateToLatest(report Reporter) error {
    prefix := "aibp-claude"
    major, _, _ := (ClaudeEnsurer{}).AIBPVersion()
...
| 7 | `npmRegistryLatest` / `semverLess` 放 ensure.go | 通用域工具（npm 语义），即使现仅 opencode 用；与 `EnsureAll` 同位 |
| 8 | 不引 semver 依赖 | aibp 包严格 X.Y.Z，10 行内联足够；YAGNI |
...
| # | 文件 | 改动 | 依赖 |
|---|------|------|------|
| 1 | `internal/aibp/ensure_agents/ensure.go` | `AgentEnsurer` 加 `UpdateToLatest()`；新增 `UpdateAll` + `npmRegistryLatest` + `semverLess`；import `net/http` `encoding/json` `strconv` `time` | 无 |
| 2 | `ensure_pi.go` | 加 `UpdateToLatest`（report "self-manages, skipping" + return nil） | 1 |
| 3 | `ensure_opencode.go` | 加 `UpdateToLatest`（3.3）；`opencodeRemoveTuiEntries` 谓词加一行（含 `aibp-agents`）；`opencodeNpmAIBPVersion` 顺带返回 `version` | 1 |
...
| 7 | `README.md` / `CHANGELOG.md` | README 加 `--update-aibp` 说明；CHANGELOG `[Unreleased]` 加 **Added** 条目 | 1-6 |

改动量：~90 行净增（opencode/claude 的 `UpdateToLatest` + `npmRegistryLatest` + `semverLess`），**零新增 helper、零删除**，仅 2 处现有代码微调（谓词泛化 1 行 + 返回值扩展）。每步后 `make build-quick`，全完成后 `make build`。

现有 `_test.go`（ensure_pi/opencode/claude）用具体类型（`OpencodeEnsurer{}` 等）直接调方法、不实现 `AgentEnsurer` 接口，故新增方法**不会**让它们编译失败（接口断言 `var _ AgentEnsurer = …` 在主 `.go` 文件里，由步骤 2-4 补齐方法后仍成立）。可选：给 `UpdateToLatest` 加表驱动测试（用 fake registry / mock exec）。
...
- **D25**（`AgentEnsurer` 接口）：接口扩展一个方法（`UpdateToLatest`），`Ensure` / `EnsureAll` 编排不变。opencode 侧只微调 `opencodeRemoveTuiEntries` 谓词 + `opencodeNpmAIBPVersion` 返回值，`installOrUpdate` / `uninstallAIBP` 复用不变。
