# D17 · microNeo 通过 `:check-aibp` 命令初始化 aibp-pi
...
用户下载安装 microNeo 后，运行一次 `:check-aibp` 命令完成 aibp-pi 的检查与（必要的）安装，之后按 Alt-Enter 就能让 notePane 跟 pi 通信。
...
**用户怎么知道要运行 `:check-aibp`**：首次按 Alt-Enter 时，`aibp.Discover()` 找不到 receiver，InfoBar 提示 `✗ no receiver found`，notePane 不打开。这个已有的守卫（`notepane.go:207-208`）就是自然引导信号。用户可据此查找文档或运行 `:check-aibp`。

**不做的事情**：启动时自动检查（用户主动跑 `:check-aibp`）、git 源安装、运行时弹窗问用户、扩展过旧时自动联网升级（我们只检测 + 提示，更新是用户自己的事，见 D9）。

---

## 二、方案选型

**npm 标准分发**：aibp-pi 作为独立 npm 包发布（`npm:aibp-pi`，已上线）。用户运行 `:check-aibp` 时检查 pi 的 `settings.json`，若没有登记 aibp-pi 就调 `pi install` 装上。
...
**命令触发**：用户主动运行 `:check-aibp`，而非启动时自动检查。用户主动触发，避开启动期的 UI 时序问题（主循环未起时 InfoBar 渲染不出来），且给明确反馈天经地义。
...
| **microNeo 自有命令** | `:check-aibp` 等命令方法 + 注册 | `internal/action/command_neo.go`（对应上游 `command.go`） |

```
internal/
├── action/
│   ├── globals.go          ← InitGlobals() 末尾调 RegisterCommands()（+1 行，该文件已被 notePane 改动）
│   └── command_neo.go          ← ⭐ microNeo 自有命令（:check-aibp 等）+ 注册，对应上游 command.go
└── aibp/
...
`command.go` 是纯上游 micro 文件（git 历史确认未受 microNeo 改动）。micro 提供了 `MakeCommand(name, action, completer)` 专门给插件/扩展注册命令，正是为避免侵入 `InitCommands` 的 map。microNeo 调用它注册 `:check-aibp`，零修改 `command.go`。
...
| `MajorVersion(扩展协议)` vs `MajorVersion(microNeo 协议)` | 含义 | `:check-aibp` 行为 |
|----------------------------------------------|------|-------------|
...
- **幂等**：重复运行 `:check-aibp`，已装且兼容则只读不 spawn，提示"就绪"
...
| # | 决策 | 理由 |
|---|------|------|
| D1 | **触发方式：用户运行 `:check-aibp` 命令** | 用户主动触发，避开启动期的 UI 时序问题；用户需知道命令，由 Alt-Enter 失败时的 `no receiver found` 提示自然引导 |
...
| D8 | **协议版本静态化**：扩展 `package.json` 的 `aibp.protocol` 是单一事实来源 | `:check-aibp` 静态读它判断兼容性；运行时也读它写注册表。静态/运行时必然一致（详见 [`说明-AIBP §7.1.1`](./说明-AIBP.md)） |
...
**② `internal/action/command_neo.go`** —— microNeo 自有命令文件（对应上游 `command.go`）：`:check-aibp` 方法 + 注册
...
**Build gate**：`make build` 过；全新环境运行 `:check-aibp` 后 `~/.pi/agent/settings.json` 含 `npm:aibp-pi`、`pi list` 显示 aibp-pi、Alt-Enter 能打开 notePane。
...
| # | 场景 | 前置条件 | 步骤 | 期望 |
|---|------|---------|------|------|
| T1 | 未装 pi | PATH 去掉 pi | 运行 `:check-aibp` | InfoBar 提示"未检测到 pi，请先安装"；不改 settings.json |
| T2 | 首次安装 | 全新环境，pi 在 PATH | 运行 `:check-aibp` | settings.json 含 `npm:aibp-pi`（unpinned）；`pi list` 显示 aibp-pi；InfoBar 提示"就绪" |
| T3 | 常态快路径 | T2 后再运行 | 运行 `:check-aibp` | 不 spawn install；InfoBar 提示"就绪" |
| T4 | 登记丢失 | T2 后 `pi remove npm:aibp-pi` | 运行 `:check-aibp` | 重新 `pi install`；`pi list` 重现 aibp-pi |
| T5 | pi install 失败 | PATH 放一个无 install 子命令的假 pi | 运行 `:check-aibp` | InfoBar 提示升级 pi |
| T6 | 扩展协议过旧 | 改已装 package.json 的 `aibp.protocol` 为 `"aibp-0"` | 运行 `:check-aibp` | InfoBar 提示运行 `pi update`（**D9：不自动升级**） |
| T7 | microNeo 协议过旧 | 改已装 package.json 的 `aibp.protocol` 为 `"aibp-2"` | 运行 `:check-aibp` | InfoBar 提示升级 microNeo |
| T8 | 端到端 | T2 后 | 开 pi → Alt-Enter → 写一行 → Alt-Enter | notePane 打开；pi 收到消息 |
| T9 | 幂等 | T2 后连运行两次 | 第二次 `:check-aibp` | 不 spawn install；都提示"就绪" |
| T10 | Alt-Enter 自然引导 | 未装 aibp-pi | 按 Alt-Enter | InfoBar 提示 `✗ no receiver found`；notePane 不打开 |
| T11 | pinned 历史遗留 | 手动 `pi install npm:aibp-pi@1.0.0` 后改协议为 aibp-0 | 运行 `:check-aibp` | InfoBar 提示扩展协议过旧 |
| T12 | 自定义 PI_CODING_AGENT_DIR | 设 `PI_CODING_AGENT_DIR=/tmp/fake_pi`，启动 pi 装好 aibp-pi（装到该路径下） | 运行 `:check-aibp` | `HasAIBP` 返回 true（读到 `/tmp/fake_pi/settings.json`）；不重装；InfoBar 提示"就绪" |
| T13 | 已装但 package.json 损坏 | T2 后删除/损坏 `<piAgentDir>/npm/node_modules/aibp-pi/package.json` | 运行 `:check-aibp` | 视作"没装"→触发 `Install` 重装；重装后提示"就绪" |
...
- [ ] T1–T13 全部通过
- [ ] 端到端：全新环境运行 `:check-aibp` 自动装好 aibp-pi，之后 Alt-Enter 能发消息
...
func RegisterCommands() {
	MakeCommand("check-aibp",          (*BufPane).CheckAibpCmd,          nil) // 检查 pi
	MakeCommand("check-aibp-opencode", (*BufPane).CheckAibpOpencodeCmd, nil) // 未来
}
