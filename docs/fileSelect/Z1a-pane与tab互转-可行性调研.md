...
- `:tab <file>`：用同一 buffer 重开 Tab（buffer 共享，但路径绕一圈，新 Tab rect按 tab-list 默认）
...
| API | 行 | 作用 |
|---|---|---|
| `NewTabFromPane(x, y, w, h int, pane Pane) *Tab` | 259 | 把任意 pane 包成新 Tab（带rect） |
...
| `(*Tab).Resize()` | 380 | 遍历 panes 对齐节点rect |
...
microNeo 在 `command_neo.go:112-117`（`AddTab`）和 `:141-146`（`NewTabCmd`）手动补了 `MainTab().Resize()` —— micro 原生 `Tabs.AddTab` 不会重新触发rect对齐，新 Tab 加进去后 src Tab 的 split tree rect可能滞后。
...
整个过程涉及三种数据结构的协同变化：split tree rect、`Tab.Panes` 列表、`TabList` 列表与 active 索引。micro 不缺任何一个 API，只缺一个把它们拼起来的上层 action + 用户键位绑定。键位选型见独立 PLAN 文档。
