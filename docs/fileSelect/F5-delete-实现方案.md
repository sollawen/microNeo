...
```text
文件：Delete file <filename>?
目录：Delete folder [dirname] and all its contents?
```

尖括号和方括号均为实际显示字符，`filename`、`dirname` 分别替换为目标名称。Dialog title 使用 `Delete`。
...
func (fm *Session) startDelete() {
	...
	anchor := dialog.Pos{
		X: fm.rect.X + 1, // 与 rename InputDialog 一致：左右各离 finder 边框 1 格
	...
	dlg.Open(
		...
		"Delete",
		anchor,
		s.pickerW - 2, // 与 rename InputDialog 一致
		dialog.AlignCenter,
		0,
		config.DefStyle,
		func(confirmed bool) {
...
1. 光标停在普通文件上按 `d`，ConfirmDialog 单行显示 `Delete file <filename>?`。
2. 光标停在目录上按 `d`，ConfirmDialog 单行显示 `Delete folder [dirname] and all its contents?`。
3. 选择 Cancel、按 Esc，文件仍存在。
4. 选择 OK，文件被删除，finder 立即刷新。
5. 删除空目录成功。
6. 删除含文件和子目录的非空目录成功。
7. 光标在面包屑上按 `d` 无反应。
8. 删除无权限目标时显示 Error MsgDialog，列表不变。
9. ConfirmDialog 打开期间 resize，不删除目标且 finder 正常关闭。
10. 删除 git 仓库中的文件后，刷新后的 git 标志正确更新。
