...
func (fm *Session) Open(rect Rect, cwd, file string, isQuit bool, onClose func(Result)) bool // false=预检放不下，不开会话、不触发 onClose
func (fm *Session) IsOpen() bool
func (fm *Session) HandleEvent(event tcell.Event) // 拦截 EventResize → close(Resize)，其余转 handleKey
func (fm *Session) NotifyBlur()                    // owner 失焦通知（非 tcell 事件，走方法不走管道）：等价 Esc 取消
...
- finder **不碰 `InfoBar`**：原 `computeLayout` 在窄窗时直接调 `InfoBar.Message`（`fileselector.go:365/369`），搬包后够不到。预检放不下走 Open 返回值 false（不开会话），提示由 owner 侧（N0b 的 `OpenFinder`）拿到 false 后自己调 InfoBar（讨论 §A.2）。
...
| —（旧 FloatFrame 在主循环全局屏蔽鼠标，焦点不会从 owner 移走） | `NotifyBlur()` → `fm.close(Esc)`（owner 失焦时直接调方法，非 tcell 事件） |

Owner 调用改为 `h.finder.Open(...)` + `h.finder.HandleEvent()` / `h.finder.Display()` / 失焦时 `h.finder.NotifyBlur()`，不再通过 `FloatOpenSpec` 中转。

**失焦关闭（新增路径，非迁移）**：旧实现寄生 FloatFrame，鼠标在主循环（`micro.go:585`）就被全局拦下、`Tabs.HandleEvent` 根本不跑，owner 焦点不会移走。finder 升格为 owner-local overlay 后，鼠标走 `Tab.HandleEvent`（`tab.go:290-336`：点 pane 切焦点 / 拖分割条 / 滚轮），这些路由在 `owner.HandleEvent` 之**上**，owner 头部的 modal `return` 拦不住——用户点别的 pane 会让 owner `SetActive(false)`、finder 孤儿化（仍 `IsOpen`、仍被画，但收不到事件）。修法：finder 暴露 `NotifyBlur()`（走方法不走 `HandleEvent` 管道——micro 从不自定义 tcell.Event，失焦是 micro 内部信号、不是终端事件），owner 失焦时经 fileops.go 调它，finder 自决 `close(Esc)`。owner 侧复用已有的 `BufPane.SetActive`（`bufpane.go:717`，其失焦分支本来就空着），接线在 N0b 步骤 1。`close(Esc)` 触发的 onClose 分支恒 no-op，不在 `Tab.SetActive` 的 pane 循环里引发突变，重入安全。
...
- `SelectResult{Kind: Closed, Reason: ReasonResize}` → `Result{Reason: Resize}`（`ReasonSize` 不再映射 Result 常量——旧 onSelect(ReasonSize) 即 Open 内预检放不下，改走新设计 `Open` 返回 `false`，不触发 onClose）
...
// Open 预检放不下: return false(不开会话、不触发 onClose; owner 自己决定 quit/no-op)
```

旧 `FileSelector` 共 **8 个 `SelectResult` 生产点**，分两类：6 处 `fs.finish(...)` 直调用（`fileselector.go:388/764/767/777/826/831`，会话开起后的关闭：resize/esc/quit/picked）+ 2 处 `onSelect(...)` 直回调（`188/199`，Open 入口预检放不下、浮窗还没开就拒掉）。注意 188/199 **不是** `fs.finish`（finish 要先关浮窗，这两处浮窗从没开过）。搬进 finder 后两类处理**分开**：finish 各点按 reason 映射成 `fm.close(<reason>)`（进 onClose）；188/199 是 Open 内预检放不下——走新设计 `Open` 返回 `false`（**不开会话、不触发 onClose**），由 owner 侧 `OpenFinder` 自己处理（browse no-op / quit 直接 Quit）。Pane mutation（`OpenCmd` / `Quit`）由旧回调（`onSelect` 在 action 的 openBrowseSelector 等函数里定义）迁移到 N0b 的 `onFinderClose`。步骤 2 只负责把新契约在 finder 包里定好，不涉及 action 侧的回调实现。
...
// NotifyBlur：owner 失焦通知（非 tcell 事件，走方法不走 HandleEvent 管道）
...
| `internal/finder/session.go` | **新增**（`FileSelector` → `Session` + state + display + handleEvent；`Rect`/`CloseReason`/`Result`；纯值 `Open`；自画 overlay + resize 拦截 + `NotifyBlur` + 预检；`NewSession`） |
...
- **失焦通知依赖**：`NotifyBlur()` 是 owner 在 `SetActive(false)` 时主动调的普通方法（非 tcell 事件，不经 `HandleEvent`）——micro 从不自定义 tcell.Event，终端输入走 `PollEvent`/`HandleEvent` 管道，内部信号走方法/回调。owner 侧接线见 N0b 步骤 1。未来若 micro 引入 tcell 原生 `EventFocus`（终端窗口焦点），那是另一个语义层，不要和本 pane 级失焦信号混。
