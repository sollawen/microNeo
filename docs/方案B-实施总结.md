...
>
> **时间线**：v1.0.5（2026-06-15，viewportRowmap 影子数组）→ v1.0.6（2026-06-16，screenBuffer 单一数据源）→ v1.0.6 Bug #7 修复（2026-06-17，case A/C 判定改用可见视口）→ v1.0.6 Bug #8 修复（2026-06-17，装饰行向下映射 + case 边界左闭右闭）
...
| | |
|---|---|
| **现象** | 光标在 line 54（小表格上方，恰好处于 scrollMargin），按 ↓ 后光标准确进入表格段（editMode 切换正确），但整个内容大幅 scroll up，StartLine 从 L23 跳到 L51（跳了 28 行，本该只 scroll up 1 行） |
| **假根因** | case C 的 `displayStart = c.Line - scrollmargin` 算错 |
| **真根因** | case A/C 判定 `curRow < height` 用 cursor 在 sb 的**绝对 vY**，隐含假设"sb 第一屏 = `[0, height)`"。但 scrollup 发生后 blit `startVY > 0`（StartLine 在 sb 内部推进，sb 本身不重画），可见窗口实际是 `[startVY, startVY+height)`。bug 帧旧 sb 的 blit startVY=2，L54 在 sb 的 curRow=37，`37 < 37` 误判 false → 走 case C。而 37 其实落在可见窗口 `[2, 39)` 内，本该走 case A |
| **修复** | 判定改为"sb 当前可见视口"：先 `startVY = sb.rowIndexOf(w.StartLine)` 拿到 blit 偏移，再用 `curRow >= startVY && curRow < startVY+height` 判断。这是旧判定的**严格细化**——startVY=0 时两者等价（不破坏 Bug #2 修复和现有场景），startVY>0 时才是正确的 |
| **教训** | **"第一屏"假设依赖 blit 偏移=0**。任何"屏幕/可见"判断都要先算出当前 blit 偏移，用相对窗口而非绝对第一屏。另外：**单场景验证通过 ≠ 所有场景通过**——Bug #2 用 goto 验证 `curRow<height` 有效，但没测"scrollup 后继续 ↓"，留下盲区。修复后要枚举所有触发 scrollup 的路径（连续 ↓、鼠标滚轮、search）回归 |

### Bug #8 — 点击表格上框装饰行 → 光标跳远 + viewport 大幅上移

| | |
|---|---|
| **现象** | `docs/MKT-plan.md`：viewport 最底一行是表格上框装饰行（line 38 空行与 line 39 表格之间）。鼠标点击它 → 光标错误跳到远处、整个 viewport 大幅 scroll up，StartLine 从 L16 跳到 L35 |
| **根因（两段）** | **① 点击映射**：`ScreenRowToLine` 对装饰行返回 `(0, false)`，调用方 `LocFromVisual` 把 false 理解成"查不到→原生 1:1 兜底"，而 1:1 在 MD 场景完全错误（忽略表格/装饰行展开），算出离装饰行很远的 buffer 行。**② relocate case 边界**：修复 ① 后光标正确落到内容行 L38（sb row 37），但 case 判定用左闭右开 `[startVY, startVY+height)=[0,37)`，`37` 恰好被排除 → 走 case C 大跳（`displayStart = c.Line - scrollmargin = 35`） |
| **修复** | **① `ScreenRowToLine`**：装饰行（line=-1）改为**向下映射到紧邻内容行**（符合从上往下读的直觉）；装饰行在 sb 渲染末尾无后续内容 → 文件 last line；空白填充（-2）仍返回 false 走原生 Scroll。**② `relocateVerticalMD` case 边界**：从左闭右开 `[startVY, startVY+height)` 改为左闭右闭 `[startVY, startVY+height]`（`<` → `<=`），让"cursor 刚好越出底部 1 行"也走 case A 小幅 scrollup |
| **教训** | **（a）fallback 不能制造假映射**：返回 `(0, false)` 的语义是"无映射"，调用方一旦用兜底算法补上，等于凭空生成一个错误的 buffer 位置。装饰行本就该有明确落点（向下最近的可见内容），不该留给兜底。（b）**边界开闭语义要匹配"刚越出"场景**：连续导航的 cursor 永远停在 `botMarginRow`，不会贴边界；贴边界的是"点击底部装饰行→向下映射"这类边缘场景，右闭才能正确触发小幅 scrollup |
...
| **"第一屏"假设依赖 blit 偏移=0** | 用 `[0, height)` 判断可见性，但 scrollup 后 blit startVY>0，可见窗口实际是 `[startVY, startVY+height)` | 任何"屏幕/可见"判断先算 blit 偏移（`rowIndexOf(StartLine)`），用相对窗口而非绝对第一屏 |
| **fallback 制造假映射 / 边界开闭语义** | 装饰行返回 `(0,false)` 被调用方用 1:1 兜底补成错误位置；case 边界左闭右开把"刚越出 1 行"误判为 case C 大跳 | 装饰行给明确落点（向下最近内容行/文件尾）；可见窗口判定用左闭右闭 `[startVY, startVY+height]` |
